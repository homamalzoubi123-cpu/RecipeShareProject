...
const HeaderSearch: React.FC<HeaderSearchProps> = ({ }: HeaderSearchProps) => {
    const { user, token, logout } = useContext(AuthContext) as AuthContextType;
    const [allUser, setAllUser] = useState<string>("");
    const [searchResults, setSearchResults] = useState<SearchUser[]>([]);
    const [searchResipes, setSearchResipes] = useState<SearchRecipe[]>([]);
    ...
    const handleSearch = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const response = await fetch(
                `${API_BASE_URL}/api/recipes/search?query=${encodeURIComponent(value)}`,
                {
                    headers: {
